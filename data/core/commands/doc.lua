...
local function save_with_io(filename)
  ...
  fp:close()
  current_doc:set_filename(normalized_filename, abs_filename)
  current_doc.new_file = false
  current_doc:clean()
  core.log("Saved \"%s\"", normalized_filename)
end
...
local function save(filename)
  core.log("Saved \"%s\", 地址 \"%s\",  \"%s\"", filename, doc().abs_filename, doc().filename)
...
local function show_save_dialog(default_filename)
  local location = default_filename
  if location then
    location = common.home_expand(location)
    -- if it's just a filename without directory, put it in home dir
    if not location:match("[/\\]") then
      location = common.home_expand("~/" .. location)
    end
  end
  core.save_file_dialog(core.window, function(status, result)
    if status == "accept" and result and result[1] then
      save(common.home_expand(result[1]))
    elseif status == "error" then
      core.log("Save dialog error: %s", tostring(result))
    end
  end, {
    title = "Save File",
...
local commands = {
  ...
  ["doc:save-as"] = function(dv)
    ...
    if dv.doc.filename then
      default_filename = dv.doc.filename:match("[^/\\]+$") or dv.doc.filename
    elseif last_doc and last_doc.abs_filename then
      default_filename = last_doc.abs_filename:match("[^/\\]+$") or "untitled.txt"
    else
      default_filename = "untitled.txt"
    end

    -- Use Windows save dialog on Windows platform
    if PLATFORM == "Windows" then
      core.log("保存到这里win'doc:save-as' call: default_filename=%s", default_filename)
      show_save_dialog(default_filename)
    else
      -- Non-Windows: use command line input
      core.log("保存到这里command'doc:save-as' call: default_filename=%s", default_filename)
