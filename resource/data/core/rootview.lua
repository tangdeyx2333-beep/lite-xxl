...
local function write_instance_storage(key, value)
  common.mkdirp(get_ws_storage_dir())
  local fp = io.open(get_ws_storage_dir() .. PATHSEP .. key, "w")
    ...
    if button == "left" then
      if self.dragged_node.dragging then
        ...
        if node and not node.locked
          ...
          local width, height = system.get_window_size(core.window)
          local mx, my = self.mouse.x, self.mouse.y
          if mx < 0 or mx > width or my < 0 or my > height then
            local view = dragged_node.views[self.dragged_node.idx]
            if view:is(DocView) and view.doc then
              local exec_cmd
              local project_dir = (core.root_project() and core.root_project().path) or EXEDIR
              if view.doc.abs_filename and not view.doc:is_dirty() then
                ...
                -- unsaved/dirty doc: create snapshot and restore in new instance
                  local snapshot_dir = USERDIR .. PATHSEP .. "unsaved_instances"
                  common.mkdirp(snapshot_dir)
                  local snapshot_name = string.format("instance_%013d_%06x.lua",
                    math.floor(system.get_time() * 1000),
                    math.random(0, 0xffffff))
                  local snapshot_path = snapshot_dir .. PATHSEP .. snapshot_name
                  local fp = io.open(snapshot_path, "w")
                  if fp then
                    local snapshot_id = snapshot_name:sub(1, -5)
                    local storage_key = string.format("%s_doc_%03d", snapshot_id, 1)
                    local doc_entry
                    ...
                    fp:close()
                    exec_cmd = string.format("%q --project-dir %q --restore-snapshot %q", EXEFILE, project_dir, snapshot_path)
                  end
                end
              end
              if exec_cmd then
                system.exec(exec_cmd)
                dragged_node:remove_view(self.root_node, view)
                self.root_node:update_layout()
                core.redraw = true
              end
