...
local function show_save_dialog(initial_filename)
  -- Use Lite XL's built-in save_file_dialog
  -- This calls system.save_file_dialog which uses native Windows dialog
  local filename
  local result

  -- Create a callback to capture the result
  local function on_save_file(status, result_filename)
    if status == "accept" and result_filename then
      filename = result_filename
    end
  end

  -- Call system.save_file_dialog
  local options = {
    title = "Save File",
    filters = { {"All Files", "*.*"} },
  ...
  -- Note: system.save_file_dialog is asynchronous
  -- For sync behavior, we use a simple approach
  core.log("show_save_dialog call: initial_filename=%s", tostring(initial_filename))
  local success, err = pcall(system.save_file_dialog, core.window, on_save_file, options)
...
function Doc:new(filename, abs_filename, new_file, skip_load)
  self.new_file = new_file
  self:reset()
  if filename then
    self:set_filename(filename, abs_filename)
    if not new_file and not skip_load then
...
function Doc:load(filename)
  ...
  local was_cancelled = self.loading_cancelled
  self:reset()
  self.loading = was_loading
  self.is_large_file = was_large_file
  self.disable_undo = was_disable_undo
  self.loading_cancelled = was_cancelled
  ...
  local function maybe_yield(force)
    ...
    if force or bytes_since_yield >= chunk_size or (system.get_time() - core.frame_start) >= time_budget then
      ...
      if core.async_debug then
        core.async_debug(
          "large-file-yield file=%s bytes=%s/%s lines=%s pending=%s",
          tostring(self.abs_filename),
          tostring(self.loading_progress_bytes),
          tostring(self.loading_total_bytes),
  ...
  while true do
    ...
    self.loading_progress_bytes = self.loading_progress_bytes + #chunk
    bytes_since_yield = bytes_since_yield + #chunk
    ...
    while true do
      ...
      append_line(pending:sub(start, nl - 1))
      bytes_since_yield = bytes_since_yield + (nl - start + 1)
      start = nl + 1
      maybe_yield(false)
      if self.loading_cancelled then
        fp:close()
    ...
    bytes_since_yield = bytes_since_yield + #pending
    maybe_yield(false)
  end
...
function Doc:save(filename, abs_filename)
  ...
  if not filename and not self.filename and not self.abs_filename then
    if PLATFORM == "Windows" then
      local selected = show_save_dialog("untitled.txt")
      if not selected then
        return -- User cancelled
      end
      filename = core.normalize_to_project_dir(selected)
      abs_filename = core.project_absolute_path(filename)
    else
      -- Non-Windows: fall back to error
      error("Cannot save unnamed document without filename")
      return
    end
  end
