...
local function open_file_and_switch_project(filename)
  ...
  local function open_selected_file()
    if not core.root_project() or core.root_project().path ~= project_dir then
  ...
  if core.root_project() and core.root_project().path ~= project_dir then
    core.confirm_close_docs(core.docs, open_selected_file)
  else
    open_selected_file()
  end
end
  ...
  if use_dialog then
    core.open_file_dialog(core.window, function(status, result)
      if status == "accept" then
        ...
        if filename then
          open_file_and_switch_project(filename)
        end
