...
function Scrollbar:_overlaps_normal(x, y)
  ...
  local interaction_margin = style.scrollbar_interaction_margin or 0
  local hit_left = sx - scrollbar_margin - interaction_margin
  local hit_right = sx + sw + interaction_margin
  local result
  if x >= hit_left and x <= hit_right and y >= sy and y <= sy + sh then
    result = "thumb"
  else
    sx, sy, sw, sh = self:_get_track_rect_normal()
    hit_left = sx - scrollbar_margin - interaction_margin
    hit_right = sx + sw + interaction_margin
    if x >= hit_left and x <= hit_right and y >= sy and y <= sy + sh then
      result = "track"
    end
  end
