...
local function is_ephemeral_workspace_path(path)
  return is_path_in_dir(path, USERDIR .. PATHSEP .. "drag_temp")
    or is_path_in_dir(path, USERDIR .. PATHSEP .. "unsaved_instances")
end

local function is_restorable_workspace_doc(doc)
  if not doc or not doc.filename or doc.filename == "" then
    return false
  end
  local abs = doc.abs_filename or doc.filename
  return not is_ephemeral_workspace_path(abs)
end
...
local function consume_workspace(project_dir)
  for key, id in workspace_keys_for(project_dir) do
    ...
    if workspace and is_ephemeral_workspace_path(workspace.path) then
      storage.clear(STORAGE_MODULE, key)
    elseif workspace and workspace.path == project_dir then
      storage.clear(STORAGE_MODULE, key)
      return workspace
    end
  end
...
local function save_view(view)
  ...
  if mt == DocView then
    if not is_restorable_workspace_doc(view.doc) then
      return nil
    end
    return {
      type = "doc",
...
local function load_view(t)
  if t.type == "doc" then
    if not t.filename or is_ephemeral_workspace_path(t.filename) then
      return nil
    end
    local dv
